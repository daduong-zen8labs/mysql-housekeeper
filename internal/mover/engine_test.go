...
func TestMoveTableDryRun(t *testing.T) {
	...
	tr, err := e.moveTable(context.Background(), 1, tcfg, true, func() time.Time { return fixed }, discardLogger())
	if err != nil {
		t.Fatal(err)
	}
...
func TestMoveTableRealBatch(t *testing.T) {
	...
	tr, err := e.moveTable(context.Background(), 9, tcfg, false, func() time.Time { return fixed }, discardLogger())
	if err != nil {
		t.Fatal(err)
	}
